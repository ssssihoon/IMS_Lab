...
def gaze(frame, points):
    ...
    if transformation is not None:
        ...
        (eye_pupil2D, _) = cv2.projectPoints(
            (int(S[0]), int(S[1]), int(S[2])), rotation_vector, translation_vector, camera_matrix, dist_coeffs
        )
        ...
        # Ensure gaze coordinates are within the frame boundaries
        gaze_x = clamp(gaze[0], 0, frame.shape[1] - 1)
        gaze_y = clamp(gaze[1], 0, frame.shape[0] - 1)

        # Draw gaze line into the screen
        p1 = (int(left_pupil[0]), int(left_pupil[1]))
        p2 = (int(gaze_x), int(gaze_y))
        cv2.line(frame, p1, p2, (0, 0, 255), 2)

        return (p2, left_pupil, right_pupil)
